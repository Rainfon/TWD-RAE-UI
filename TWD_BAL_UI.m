function TWD_BAL_UI_BatchFeedback_TightTop_NoLNoNs_Fit
% TWD_BAL_UI_BatchFeedback_TwoRow  Interactive TWD-BAL batch-feedback multicriteria sorting demo.
%
% Data format:
...
%   4. Let TWD-BAL recommend a batch of alternatives to query.
%   5. Submit the decision maker's batch class feedback and update the model.
    ...
    fig = uifigure('Name', 'TWD-BAL Interactive Multicriteria Sorting System', ...
        'Position', [100 100 1500 850], 'Color', theme.bg);

    root = uigridlayout(fig, [3 1]);
    setBackgroundIfAvailable(root, theme.bg);
    root.RowHeight = {66, '1x', 78};
    root.ColumnWidth = {'1x'};
    root.Padding = [16 12 16 4];
    root.RowSpacing = 6;
    ...
    edtXi = uieditfield(header, 'numeric', 'Value', 0.10, 'Limits', [eps 0.499], ...
    ...
    edtMAC = uieditfield(header, 'numeric', 'Value', 0.90, 'Limits', [eps 1-eps], ...
        'HorizontalAlignment', 'center', 'FontSize', 14, 'FontColor', [0 0 0], ...
        'ValueChangedFcn', @onSettingsChanged);
    edtMAC.Layout.Row = 2;
    ...
    edtKmax = uieditfield(header, 'numeric', 'Value', 5, 'Limits', [1 inf], ...
    ...
    advancedOpen = false;
    advancedControls = {lblModelParams, lblKmaxParam, edtKmax, lblXiParam, edtXi, lblMACParam, edtMAC};
    fig.UserData = state;
    ...
    summaryGrid = uigridlayout(summaryPanel, [3 1]);
    setBackgroundIfAvailable(summaryGrid, theme.panel);
    summaryGrid.RowHeight = {42, 24, '1x'};
    ...
    lblChartTitle.Layout.Row = 2;

    axRegions = uiaxes(summaryGrid);
    axRegions.Layout.Row = 3;
    ...
    feedbackSlotGrid.Padding = [0 0 0 0];
    ...
    bottom = uigridlayout(feedbackFrame, [2 2]);
    setBackgroundIfAvailable(bottom, theme.panel);
    bottom.RowHeight = {20, 46};
    bottom.ColumnWidth = {'1x', 76};
    bottom.Padding = [10 3 10 1];
    bottom.RowSpacing = 0;
    bottom.ColumnSpacing = 8;

    lblQueryHeading = uilabel(bottom, 'Text', '', ...
        'FontWeight', 'bold', 'FontSize', 13, 'FontColor', theme.text, ...
        'HorizontalAlignment', 'left');
    lblQueryHeading.Layout.Row = 1;
    lblQueryHeading.Layout.Column = [1 2];

    batchPanel = uipanel(bottom, 'BorderType', 'none', 'BackgroundColor', theme.panel);
    batchPanel.Layout.Row = 2;
    batchPanel.Layout.Column = 1;
    batchFeedbackDropdowns = gobjects(0,1);

    submitControlPanel = uipanel(bottom, 'BorderType', 'none', 'BackgroundColor', theme.panel);
    submitControlPanel.Layout.Row = 2;
    submitControlPanel.Layout.Column = 2;
    submitControlGrid = uigridlayout(submitControlPanel, [2 1]);
    setBackgroundIfAvailable(submitControlGrid, theme.panel);
    % Keep the submit control in the top row; the flexible second row absorbs
    % any remaining height so the button is no longer vertically centered.
    submitControlGrid.RowHeight = {30, '1x'};
    ...
    btnSubmit.Layout.Row = 1;
    btnSubmit.Layout.Column = 1;
    if isprop(btnSubmit, 'Tooltip')
        btnSubmit.Tooltip = 'Submit Batch';
    end
    ...
    function refreshSettingsSummary()
        lblSettingsSummary.Text = sprintf('Kmax = %d | $\\xi$ = %.3g | MAC = %.3g', ...
            round(edtKmax.Value), edtXi.Value, edtMAC.Value);
    end
    ...
    function updateControlState()
        ...
        hasPendingQuery = hasData && ~isempty(state.currentQueryGlobal) && all(isfinite(state.currentQueryGlobal(:)));
        ...
        try
            for kk = 1:numel(batchFeedbackDropdowns)
                setEnabled(batchFeedbackDropdowns(kk), hasPendingQuery);
            end
        catch
        end
    ...
    function setRiskDisplay(~, ~, ~)
        % Kept as a no-op for compatibility with older callbacks.
    end

    function onBatchTableEdited(~, ~)
        % Feedback values are read directly from the second-row dropdown controls when Submit is pressed.
    end
    ...
    function onLoadData(~, ~)
        ...
        try
            ...
            state.finalPred = nan(state.n, 1);
            state.pS = nan(state.n, 1);
            state.AIW = nan(state.n, 1);
            state.RRA = nan(state.n, 1);
            state.mMR = nan(state.n, 1);
            state.posPseudoGlobalIdx = zeros(0,1);
            state.posPseudoLabel = zeros(0,1);
            state.posPseudoWeight = zeros(0,1);
    ...
    function onRandomReferences(~, ~)
        try
            ...
            state.queryLog = emptyQueryLog();
            state.metricsHistory = emptyMetricLog();
            state.currentQueryGlobal = zeros(0,1);
            state.currentQueryLocal = zeros(0,1);
    ...
    function onSuggest(~, ~)
        try
            ...
            [selGlobal, selLocal] = selectNextTWDBatchQuery(state);
            if isempty(selGlobal)
                uialert(fig, tr('noCandidateMsg'), tr('noCandidateTitle'));
                return;
            end
    ...
    function onSubmitFeedback(~, ~)
        try
            state = fig.UserData;
            assertDataLoaded(state);
            if isempty(state.currentQueryGlobal) || any(~isfinite(state.currentQueryGlobal(:)))
                uialert(fig, tr('needSuggestMsg'), tr('needSuggestTitle'));
                return;
            end

            idxList = state.currentQueryGlobal(:);
            idxList = idxList(idxList >= 1 & idxList <= state.n & ~state.knownMask(idxList));
            if isempty(idxList)
                uialert(fig, tr('noCandidateMsg'), tr('noCandidateTitle'));
                return;
            end

            predBefore = state.pred(idxList);
            feedback = state.yTrue(idxList);  % Default offline demo values.
            if numel(batchFeedbackDropdowns) == numel(idxList)
                try
                    feedback = zeros(numel(idxList), 1);
                    for kk = 1:numel(idxList)
                        feedback(kk) = parseClassItem(batchFeedbackDropdowns(kk).Value);
                    end
                catch ME
                    error('Invalid feedback values in the batch panel: %s', ME.message);
                end
            end
            corrected = predBefore ~= feedback;
            oldQueryCount = state.queryCount;

            state.knownMask(idxList) = true;
            state.queriedMask(idxList) = true;
            state.yKnown(idxList) = feedback;
            state.queryCount = state.queryCount + numel(idxList);
            state.correctionCount = state.correctionCount + sum(corrected);

            newLog = table( ...
                (oldQueryCount + (1:numel(idxList)))', ...
                string(state.altNames(idxList)), ...
                predBefore(:), ...
                feedback(:), ...
                state.yTrue(idxList), ...
                corrected(:), ...
                string(state.region(idxList)), ...
                state.pS(idxList), ...
                'VariableNames', {'Round','Alternative','PredBefore','FeedbackClass','TrueClass','Corrected','Region','pS'});
    ...
    function refreshDataTable()
        ...
        if isfield(state, 'finalPred') && numel(state.finalPred) == state.n && any(isfinite(state.finalPred(poolIdx)))
            D.Class = cellstr(displayClassNames(state.finalPred(poolIdx)));
        else
    ...
    function refreshMetricsTables()
        ...
        if ~isempty(state.queryLog)
            tblLog.ColumnWidth = {70, 120, 112, 112, 88, 90, 80, 80};
        end
        try
            removeStyle(tblLog);
    ...
    function refreshBatchFeedbackPanel()
        delete(batchPanel.Children);
        batchFeedbackDropdowns = gobjects(0,1);

        state = fig.UserData;
        if isempty(state.X) || isempty(state.currentQueryGlobal) || any(~isfinite(state.currentQueryGlobal(:)))
            emptyGrid = uigridlayout(batchPanel, [1 1]);
            setBackgroundIfAvailable(emptyGrid, theme.panel);
            emptyGrid.Padding = [0 0 0 0];
            msg = uilabel(emptyGrid, 'Text', tr('batchEmpty'), ...
                'FontName', theme.font, 'FontSize', 12, 'FontColor', theme.muted, ...
                'HorizontalAlignment', 'center');
            if isprop(msg, 'VerticalAlignment')
        ...
        idxList = state.currentQueryGlobal(:);
        idxList = idxList(idxList >= 1 & idxList <= state.n);
        if isempty(idxList)
            emptyGrid = uigridlayout(batchPanel, [1 1]);
            setBackgroundIfAvailable(emptyGrid, theme.panel);
            emptyGrid.Padding = [0 0 0 0];
        ...
        batchSize = numel(idxList);
        g = uigridlayout(batchPanel, [2 batchSize]);
        setBackgroundIfAvailable(g, theme.panel);
        g.RowHeight = {18, 28};
        g.ColumnWidth = repmat({'1x'}, 1, batchSize);
        g.Padding = [0 0 0 0];
        g.RowSpacing = 0;
        g.ColumnSpacing = 8;

        batchFeedbackDropdowns = gobjects(batchSize, 1);
        items = classItems(state.q);
        for kk = 1:batchSize
            altLabel = uilabel(g, 'Text', char(displayAlternativeNames(state.altNames(idxList(kk)))), ...
                'FontName', theme.font, 'FontSize', 15, 'FontWeight', 'bold', ...
                'FontColor', theme.text, 'BackgroundColor', theme.neutralSoft, ...
                'HorizontalAlignment', 'center');
            altLabel.Layout.Row = 1;
            altLabel.Layout.Column = kk;

            dd = uidropdown(g, 'Items', items, ...
                'Value', char(displayClassName(state.yTrue(idxList(kk)))), ...
                'FontName', theme.font, 'FontSize', 14, 'FontColor', theme.valueRed);
            dd.Layout.Row = 2;
            dd.Layout.Column = kk;
            batchFeedbackDropdowns(kk) = dd;
        end
    end
    ...
    function setResultsTableLook()
        ...
        dataWidths = [{round(128 * widthScale)}, critWidths, ...
            {round(104 * widthScale), round(110 * widthScale), round(120 * widthScale)}];
        ...
        try
            ...
            % 6. 当前推荐询问批次不额外改色，保持 POS / BND / NEG 原区域颜色。
    ...
    function state = updateCfgFromUI(state)
        cfg = defaultCfg();
        % L is fixed at the default value cfg.L = 3.
        cfg.xi = edtXi.Value;
        cfg.MAC = edtMAC.Value;
        % Ns is fixed at the default value cfg.Ns = 100.
    ...
    function names = logColumnNames()
        if lang == "en"
            names = {'Round','Alternative','Pred. before','Feedback','True','Corrected','Region','P(S)'};
        else
            names = {'轮次','方案','询问前预测','反馈类','真实类','是否纠错','区域','P(S)'};
        end
    end
...
function setEnabled(obj, tf)
    ...
    for ii = 1:numel(obj)
        try
            if isvalid(obj(ii)) && isprop(obj(ii), 'Enable')
                obj(ii).Enable = value;
            end
        catch
        end
...
function out = translateZH(key)
    switch key
        ...
        case "afterTrain"
            out = '已训练并完成分类。点击“推荐批量询问”选择下一批高风险方案。';
        case "suggestLine"
            out = '建议询问';
        case "predLine"
            out = '推荐';
...
function out = translateEN(key)
    switch key
        case "appName"
            out = 'TWD-BAL Interactive Multiple Criteria Sorting System';
        case "title"
            out = 'TWD-BAL Interactive Multiple Criteria Sorting System';
        case "subtitle"
            out = '';
        ...
        case "queryPanel"
            out = ' ';
        case "queryHeading"
            out = 'Batch query alternatives and feedback';
        case "legendIntro"
            out = '';
        case "legendRef"
            out = 'Reference';
        ...
        case "feedbackOptions"
            out = 'Options: C₁, C₂, C₃, ...';
        case "batchEmpty"
            out = 'After suggesting a query batch, alternatives and feedback selectors appear here.';
        case "knownTotal"
            out = 'Known / Total';
        case "queryBudget"
            out = 'Query budget';
        ...
        case "afterTrain"
            out = 'Model trained and alternatives sorted. Click "Suggest Batch" for the next high-risk batch.';
        case "suggestLine"
            out = 'Suggested query';
        case "predLine"
            out = 'Predicted';
...
function state = emptyState()
    ...
    state.queriedMask = [];
    state.yKnown = [];
    state.model = [];              % base model trained only on true known/reference labels, used for TWD partition and BCM-MMR
    state.finalModel = [];         % final display/evaluation model trained with true known labels + POS pseudo labels
    state.pred = [];               % base-model prediction used for P(S), region, and correction counting
    state.finalPred = [];          % final prediction after adding POS pseudo labels
    state.pS = [];
    state.AIW = [];
    state.RRA = [];
    state.mMR = [];
...
function cfg = defaultCfg()
    % TWD-BAL default settings used in the paper-style batch method.
    cfg.L = 3;
    cfg.xi = 0.10;
    cfg.MAC = 0.90;
    cfg.Ns = 100;
    cfg.Kmax = 5;
    cfg.bcmTrajectories = 20;
...
function T = emptyQueryLog()
    T = table('Size', [0 8], ...
        'VariableTypes', {'double','string','double','double','double','logical','string','double'}, ...
        'VariableNames', {'Round','Alternative','PredBefore','FeedbackClass','TrueClass','Corrected','Region','pS'});
end
...
%TRAINANDCLASSIFYSTATE
% Fully consistent TWD-BAL update:
%   1) Train a base MM-UTADIS model only with true known/reference labels.
%   2) Use the base model to compute MS, P(S), and the strict POS/BND/NEG regions.
%   3) Store POS alternatives as temporary pseudo labels with weight P(S).
%   4) Do not query POS or BND; batch query selection is handled separately from NEG only.
%   5) Train a final display/evaluation model with true known labels + POS pseudo labels.
%      POS pseudo labels are temporary and are NOT written into knownMask.

    Xtrue = state.X(state.knownMask,:);
    ytrue = state.yKnown(state.knownMask);
    q = state.q;
    cfg = state.cfg;

    % Base model: true known/reference labels only. This model defines TWD regions.
    baseModel = trainMMUTADIS(Xtrue, ytrue, q, cfg);
    state.model = baseModel;

    state.pred = nan(state.n, 1);       % base prediction for P(S), region, correction counting
    state.finalPred = nan(state.n, 1);  % final prediction after POS pseudo labels
    state.pred(state.knownMask) = state.yKnown(state.knownMask);
    state.finalPred(state.knownMask) = state.yKnown(state.knownMask);
    ...
    if ~isempty(poolIdx)
        ...
        rob = computeRobustInfo(baseModel, Xtrue, ytrue, Xpool, cfg);

        % TWD partition and query diagnostics are always based on the base model.
        state.pred(poolIdx) = rob.pred;
        state.pS(poolIdx) = rob.pS;
        state.AIW(poolIdx) = rob.AIW;
        state.RRA(poolIdx) = rob.RRA;
        state.mMR(poolIdx) = rob.mMR;
        state.region(poolIdx) = regionNames(rob.Region);

        % POS pseudo labels: temporary, weighted by P(S), and not marked as known.
        posLocal = find(rob.Region == 1);
        if ~isempty(posLocal)
            state.posPseudoGlobalIdx = poolIdx(posLocal);
            state.posPseudoLabel = rob.pred(posLocal);
            pseudoWeight = rob.pS(posLocal);
            pseudoWeight = max(0, min(1, pseudoWeight));
            if isfield(cfg, 'minPseudoLabelWeight') && ~isempty(cfg.minPseudoLabelWeight)
                pseudoWeight = max(pseudoWeight, cfg.minPseudoLabelWeight);
            end
            state.posPseudoWeight = pseudoWeight(:);
        end

        % Final TWD-BAL model for display/evaluation: true known labels + POS pseudo labels.
        if ~isempty(state.posPseudoGlobalIdx)
            XfinalTrain = [Xtrue; state.X(state.posPseudoGlobalIdx,:)];
            yfinalTrain = [ytrue; state.posPseudoLabel(:)];
            wfinalTrain = [ones(numel(ytrue),1); state.posPseudoWeight(:)];
            finalModel = trainWeightedMMUTADIS(XfinalTrain, yfinalTrain, wfinalTrain, q, cfg);
        end

        finalPoolPred = predictModel(finalModel, Xpool);
        state.finalPred(poolIdx) = finalPoolPred;
    end

    state.finalModel = finalModel;

    % Metrics report the final TWD-BAL output, not merely the base TWD partition model.
    metrics = computeClassificationMetrics(state.yTrue, state.finalPred, q);
...
function [selGlobal, selLocal] = selectNextTWDBatchQuery(state)
%SELECTNEXTTWDBATCHQUERY
% Strict TWD-BAL query rule used in the paper:
%   1) The current unlabeled pool is first partitioned into POS/BND/NEG.
%   2) POS is accepted as pseudo-label information and is NOT queried.
%   3) BND is delayed and is NOT queried.
%   4) The batch query is constructed only from the initial NEG region.
%   5) Within NEG, BCM trajectory simulation is performed using the MMR score.
%   6) The final batch is selected from trajectory candidates by GreedyMatch.
%
% No fallback to the whole pool is allowed here; if NEG is empty, the batch is empty.
    ...
    remainingBudget = max(0, state.cfg.Kmax - state.queryCount);
    ...
    % Strict three-way decision rule: query only the NEG region.
    ...
    batchSize = min(remainingBudget, numel(negLocal));
    if batchSize <= 0
        selGlobal = zeros(0,1);
        selLocal = zeros(0,1);
        return;
    ...
    yref = state.yKnown(state.knownMask);
    Xneg = state.X(poolIdx(negLocal), :);

    % BCM-MMR batch construction inside NEG only.
    selectedNegLocal = selectBatchByOriginalBCMMMR(state.model, Xref, yref, Xneg, state.cfg, batchSize);

    selectedNegLocal = unique(selectedNegLocal(:), 'stable');
    selectedNegLocal = selectedNegLocal(selectedNegLocal >= 1 & selectedNegLocal <= numel(negLocal));
    selectedNegLocal = selectedNegLocal(1:min(numel(selectedNegLocal), batchSize));

    if isempty(selectedNegLocal)
        selGlobal = zeros(0,1);
        selLocal = zeros(0,1);
        return;
    end

    selLocal = negLocal(selectedNegLocal);
    selGlobal = poolIdx(selLocal);
end

function selected = selectBatchByOriginalBCMMMR(model0, Xtrain0, ytrain0, XpoolNeg, cfg, k)
%SELECTBATCHBYORIGINALBCMMMR
% Original BCM simulation matching using MMR as the sequential policy.
    ...
    [trajectories, initialScore] = simulateBCMTrajectoriesMMR(model0, Xtrain0, ytrain0, XpoolNeg, cfg, k, numTraj);
...
function [trajectories, initialScore] = simulateBCMTrajectoriesMMR(model0, Xtrain0, ytrain0, XpoolNeg, cfg, k, numTraj)
%SIMULATEBCMTRAJECTORIESMMR
% Each trajectory sequentially selects a high-MMR sample, samples a simulated
% label from MS, updates the simulated reference set, and continues.

    nPool = size(XpoolNeg, 1);
    trajectories = cell(numTraj, 1);

    [initialScore, ~, ~] = computeMMRStepScore(model0, Xtrain0, ytrain0, XpoolNeg, cfg);
    ...
    for trajectoryId = 1:numTraj
        ...
        for step = 1:k
            ...
            try
                ...
                [score, pred, MS] = computeMMRStepScore(currentModel, currentX, currentY, Xcand, cfg);
            catch
            ...
            chosenLocal = chooseOneByScore(score, (1:numel(currentPoolIdx))');
            ...
            currentX = [currentX; XpoolNeg(chosenPoolIdx, :)]; %#ok<AGROW>
...
function [score, pred, MS] = computeMMRStepScore(model, Xtrain, ytrain, Xcand, cfg)
...
function selected = selectBCMByDeletionGreedyMatch(candidateUniverse, trajectories, Xpool, cfg, k, scoreForTieBreak)
    ...
    PhiAll = computePhiBasis(Xpool, cfg.L);
...
function model = trainWeightedMMUTADIS(X, y, sampleWeight, q, cfg)
    ...
    [~, ~, phiStar] = solveUTADISStage1PaperWeighted(X, y, sampleWeight, q, cfg);
    [theta, b, delta] = solveUTADISStage2PaperWeighted(X, y, sampleWeight, q, phiStar, cfg);
...
function w = validateSampleWeights(w, n)
    ...
    w = double(w(:));
    if numel(w) ~= n
        error('样本权重长度不匹配：numel(w)=%d, n=%d。', numel(w), n);
    end
...
function [theta, b, phiStar] = solveUTADISStage1PaperWeighted(X, y, sampleWeight, q, cfg)
...
function [theta, b, delta] = solveUTADISStage2PaperWeighted(X, y, sampleWeight, q, phiStar, cfg)
    sampleWeight = validateSampleWeights(sampleWeight, numel(y));
...
function samples = sampleCompatibleAdditiveModels(Xref, yref, Xpool, q, phiStar, cfg)
    ...
    if nAccepted == 0
        error('相容模型采样失败：没有得到可行模型。可尝试增大 compatTol 或增加参考样本。');
    end
...
function [pred, U] = predictModel(model, X)
    switch string(model.type)
        case {'MM-UTADIS', 'Weighted-MM-UTADIS'}
